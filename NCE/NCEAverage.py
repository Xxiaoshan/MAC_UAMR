...
class NCEAverage(nn.Module):
    ...
    def __init__(self, inputSize, outputSize, K, T=0.07, momentum=0.2, use_softmax=False):
    ...
    def forward(self, l, ab, y, idx=None):
        ...
        if idx is None:
            ...
            #Bn * K (sampling length) obtained the sampling index idx by randomly sampling K+1 positive and negative samples
            idx.select(1, 0).copy_(y.data)
            #Replace the first column with the label corresponding to the label - ensuring the existence of positive samples
        # sample
        weight_l = torch.index_select(self.memory_l, 0, idx.view(-1)).detach() #Select based on sampling index
        weight_l = weight_l.view(batchSize, K + 1, inputSize)
        out_ab = torch.bmm(weight_l, ab.view(batchSize, inputSize, 1)) #Calculate similarity
        #By sampling, K+1 features from memory were extracted,
        ...
        #matrix multiplication and calculated two similarities
        ...
        with torch.no_grad():
            ...
            #Index of all samples in the current batch in the entire dataset
            l_pos.mul_(momentum)
            l_pos.add_(torch.mul(l, 1 - momentum))
            l_norm = l_pos.pow(2).sum(1, keepdim=True).pow(0.5)
            updated_l = l_pos.div(l_norm)
            self.memory_l.index_copy_(0, y, updated_l)
            #Store the updated features of all samples in the current batch in the memory bank
            #The location stored here is the index of a single sample across the entire dataset.
